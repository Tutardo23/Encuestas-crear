...
export const POST = withPublicRateLimit(
  async (req, ip) => {
    ...
    if (recipient.tokenExpiresAt && recipient.tokenExpiresAt < new Date()) return err("Este link expiró.", 410);
    if (recipient.survey.status !== "ACTIVE") return err("Esta encuesta ya no está disponible.", 410);
    if (recipient.response && !recipient.survey.allowMultipleResponses) return err("Ya respondiste esta encuesta.", 409);

    const questionIds = new Set(recipient.survey.questions.map((q: { id: string }) => q.id));
    ...
    await prisma.$transaction(async (tx: any) => {
      ...
      await tx.recipient.update({ where: { id: recipient.id }, data: { status: "COMPLETED" } });
      return resp;
    });

    await auditLog({ action: "RESPONSE_SUBMITTED", entity: "Survey", entityId: recipient.surveyId, metadata: { recipientId: recipient.id }, ip });
...
export const GET = withPublicRateLimit(
  async (req, ip) => {
    ...
    if (recipient.status === "SENT") {
      await prisma.recipient.update({ where: { id: recipient.id }, data: { status: "OPENED", openedAt: new Date() } });
    }
    ...
    return ok({
      ...
      questions: recipient.survey.questions.map((q: {
        ...
        id: q.id, type: q.type, text: q.text, isRequired: q.isRequired, order: q.order,
    ...
    void ip; // ip used for rate limiting only in GET
  },
