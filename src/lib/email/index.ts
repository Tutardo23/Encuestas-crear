...
// Resend se instancia DENTRO de la función, no al importar el módulo.
// Así el build de Next.js no explota cuando las variables no están disponibles.
...
export async function sendSurveyEmail(params: SendSurveyEmailParams) {
  ...
  // Leer variables en runtime, no en tiempo de importación
  const apiKey  = process.env.RESEND_API_KEY;
  const appUrl  = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
  const from    = `${process.env.RESEND_FROM_NAME ?? "Torx Survey"} <${process.env.RESEND_FROM_EMAIL ?? "noreply@torx.com"}>`;

  const surveyLink = `${appUrl}/r/${token}`;
  const firstName  = recipientName.split(" ")[0];

  // Modo desarrollo / sin API key configurada: solo loguear
  if (!apiKey || apiKey.startsWith("re_xxx")) {
    console.log(`[Email DEV] Para: ${to} | Link: ${surveyLink}`);
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
          <tr>
            <td style="background:#0B1220;border-radius:12px 12px 0 0;padding:28px 36px;">
              <span style="font-size:13px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;color:#fff;">
                <span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:#C84B31;margin-right:8px;vertical-align:middle;"></span>
                TORX SURVEY
              </span>
            </td>
          </tr>
          <tr>
            <td style="background:#ffffff;padding:36px 36px 28px;">
              ...
              <p style="margin:0 0 24px;font-size:16px;color:#444;line-height:1.7;">
                ...
                Solo te tomará 2 minutos y tu opinión es muy valiosa.
              </p>
